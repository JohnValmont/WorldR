...
exports.decideNpcActions = decideNpcActions;
exports.runNpcBrainForCompany = runNpcBrainForCompany;
exports.spawnNpc = spawnNpc;
const crypto = __importStar(require("crypto"));
const npc_1 = require("../constants/npc");
 ...
 */
function decideNpcActions(input) {
    const { salePrice, costPerUnit, factoryCapacity, marketingTier, awareness, availableCash, lastRevenue, unitsSoldLastArc, unitsAllocatedLastArc, marketShareThisArc, marketShareLastArc, reasonCode, zeroDemandStreak, modelAgeMonths, unitsProducedLastArc, inventoryInStock } = input;
    ...
    let desired = Math.round(unitsSoldLastArc * npc_1.PRODUCTION_BUFFER);
    ...
    let newTargetUnits = Math.max(npc_1.MIN_UNITS_FLOOR, Math.min(desired - inventoryInStock, factoryCapacity));
    ...
    if (reasonCode === 'Sold Out') {
        ...
        if (salePrice < maxAllowedPrice) {
            newSalePrice = Math.min(salePrice * (1 + npc_1.PRICE_STEP), maxAllowedPrice);
        }
        const desiredSoldOut = Math.max(Math.round(unitsSoldLastArc * (npc_1.PRODUCTION_BUFFER + 0.50)), Math.round(factoryCapacity * 0.25));
        newTargetUnits = Math.max(npc_1.MIN_UNITS_FLOOR, Math.min(desiredSoldOut - inventoryInStock, factoryCapacity));
    }
    ...
    // Also drop price if we are hoarding inventory (selling less than 50% of allocation)
        ...
        if (sellRatio < 0.2) {
            newSalePrice = salePrice * (1 - (npc_1.PRICE_STEP * 2.5));
        }
        ...
        // OR if we are just fundamentally failing to sell our stock (sellRatio < 0.5)
            ...
            newSalePrice = salePrice * (1 - npc_1.PRICE_STEP);
        }
    }
    // Ensure production limits are respected (already clamped above, but just in case)
    newTargetUnits = Math.max(npc_1.MIN_UNITS_FLOOR, Math.min(newTargetUnits, factoryCapacity));
    ...
    if (reasonCode === 'Low Brand Awareness' || awareness < npc_1.AWARENESS_BUMP_THRESHOLD) {
        ...
        if (currentTierIndex >= 0 && currentTierIndex < MARKETING_TIERS.length - 1) {
            ...
            if (nextTier.cost <= npc_1.MARKETING_REVENUE_PCT * lastRevenue && availableCash > nextTier.cost) {
    ...
    if (newZeroDemandStreak >= npc_1.ZERO_DEMAND_FACELIFT_MONTHS) {
        faceliftFlag = true;
    }
    // B5 Rule: Natural aging lifecycle of a vehicle mandates a facelift
    if (modelAgeMonths >= npc_1.MODEL_AGE_FACELIFT) {
        faceliftFlag = true;
    }
    ...
    if (unitsAllocatedLastArc > 0 &&
        reasonCode !== 'Zero Demand' &&
        reasonCode !== 'Low Brand Awareness' &&
        (unitsSoldLastArc / unitsAllocatedLastArc) >= npc_1.NPC_EXPAND_SELL_RATIO) {
        expandMarkets = true;
    }
    return {
        newSalePrice,
...
async function ensureUniqueModelName(trx, worldInstanceId, baseName) {
...
async function performIndustrialEspionage(trx, companyId, currentYear, currentMonth, availableCash) {
    const ESPIONAGE_COST = 1000000; // $1M threshold so NPCs execute espionage frequently
    if (availableCash < ESPIONAGE_COST)
    ...
    const modelsCount = await trx('manufacturing_vehicle_models')
        ...
        .count('* as count');
    if (Number(modelsCount[0].count) < 1)
        return availableCash;
    // Get active instance
    ...
    const topPlayerModel = await trx('manufacturing_sales_results as r')
        ...
        .first();
    if (!topPlayerModel || Number(topPlayerModel.total_sold) < 20)
        return availableCash;
    // Deduct cash
    ...
    const PREFIXES = ['Aurora', 'Nebula', 'Zenith', 'Apex', 'Vortex', 'Stratos', 'Nova', 'Pinnacle', 'Quantum', 'Eon'];
    const SUFFIXES = ['GT', 'Rival', 'Prime', 'X', 'Vanguard', 'Strider', 'Pulse', 'Dominator', 'Interceptor'];
    const randomPrefix = PREFIXES[Math.floor(Math.random() * PREFIXES.length)];
    const randomSuffix = SUFFIXES[Math.floor(Math.random() * SUFFIXES.length)];
    const rawModelName = `${npcCompany.name} ${randomPrefix} ${randomSuffix}`;
...
async function applyNpcFacelifts(trx, companyId, currentYear, currentMonth, availableCash) {
    const FACELIFT_COST = 1000000; // $1M threshold
    if (availableCash < FACELIFT_COST)
        return availableCash;
    ...
    for (const model of models) {
        ...
        if (ageMonths >= 2) {
            ...
            if (model.name.endsWith(' IV'))
                rawModelName = model.name.replace(' IV', ' V');
            else if (model.name.endsWith(' III'))
                rawModelName = model.name.replace(' III', ' IV');
            else if (model.name.endsWith(' II'))
                rawModelName = model.name.replace(' II', ' III');
 ...
 */
async function runNpcBrainForCompany(trx, companyId, currentYear, currentMonth) {
    ...
    for (const model of models) {
        ...
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} Start`;
        ...
        const unitsAllocatedLastArc = allocations.reduce((s, a) => s + Number(a.units_allocated), 0);
        ...
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - Fetching lastSales`;
        ...
        if (lastSales.length > 0) {
            const sum = lastSales.reduce((acc, s) => acc + parseFloat(s.market_share_estimate), 0);
            marketShareThisArc = sum / lastSales.length;
            lastRevenue = lastSales.reduce((acc, s) => acc + parseFloat(s.revenue), 0);
        }
        const unitsSoldLastArc = lastSales.length > 0 ? lastSales.reduce((acc, s) => acc + s.units_sold, 0) : 0;
        const reasonCode = lastSales.length > 0 ? lastSales[0].main_reason_code : null;
        // Fetch the previous month's production results
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - Fetching lastSnapshot`;
        ...
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - Fetching prevSales`;
        ...
        if (prevSales.length > 0) {
            const sum = prevSales.reduce((acc, s) => acc + parseFloat(s.market_share_estimate), 0);
            marketShareLastArc = sum / prevSales.length;
        }
        // Load brain memory table to track long term struggles (e.g. zero demand streak)
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - Fetching state`;
        const state = await trx('manufacturing_npc_state')
            .where({ company_id: companyId })
            .first();
        const zeroDemandStreak = state ? state.zero_demand_streak : 0;
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - Fetching inv`;
        ...
        // Calculate decision
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - decideNpcActions`;
        ...
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - update models`;
        await trx('manufacturing_vehicle_models')
            .where({ id: modelId })
            .update({ sale_price: output.newSalePrice });
        if (prodLine) {
            global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - update prodLine`;
            await trx('manufacturing_production_lines')
                .where({ id: prodLine.id })
                .update({ target_units_per_month: output.newTargetUnits });
        }
        if (allocations.length > 0) {
            global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - update allocations`;
            ...
            if (output.expandMarkets && allocations.length < npc_1.NPC_MAX_MARKETS) {
                global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - B7 expand markets`;
                const coveredMarketIds = new Set(allocations.map((a) => a.region_market_id));
                ...
                for (const newMarket of uncoveredMarkets) {
                    if (allocations.length + uncoveredMarkets.indexOf(newMarket) >= npc_1.NPC_MAX_MARKETS)
                        break;
                    await trx('manufacturing_market_allocations')
                        .insert({
                        company_id: companyId,
        ...
        // Sync NPC memory
        global.tickProgress = `Processing country: ... - Step 2: Decide (NPCs) - Company ${companyId} Model ${modelId} - merge npc_state`;
 ...
 */
async function spawnNpc(trx, personality, countryId, clock) {
    const roster = npc_1.NPC_ROSTER.find(r => r.key === personality);
    if (!roster)
        throw new Error(`Unknown NPC personality: ${personality}`);
