...
const db = knex({
  client: 'sqlite3',
  connection: { filename: './worldr.sqlite' },
...
async function run() {
  try {
    const richestPlayers = await db.raw(`
        ...
        LIMIT 10
      `);
    console.log("richestPlayers query succeeded. Found", richestPlayers.length);
    console.log(richestPlayers.slice(0, 2));
    ...
    console.log("topCompanies query succeeded. Found", topCompanies.length);
    ...
    console.log("popularCars query succeeded. Found", popularCars.length);
