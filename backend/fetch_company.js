...
client.connect().then(async () => { 
  const res = await client.query("SELECT company_id FROM manufacturing_vehicle_models WHERE name = 'Month Test Model 1'");
  console.log('Company ID:', res.rows[0].company_id); 
  
  // Actually, I'll just update the status directly so we can test the UI first.
  await client.query("UPDATE manufacturing_vehicle_models SET development_status = 'ready_to_launch' WHERE name = 'Month Test Model 1'");
