...
async function run() {
  ...
  const cos = await client.query(`
    ...
    WHERE c.name ILIKE '%Verdian%' OR c.name ILIKE '%HaulPro%' OR c.name ILIKE '%Arcas%' OR c.name ILIKE '%Aldrich%'
  `);
  ...
  const prod = await client.query(`
    SELECT c.name as company, p.status, SUM(p.target_units_per_arc) as target_units
  ...
  const alloc = await client.query(`
    ...
    WHERE c.id IN (${ids}) AND a.region_market_id = 'drennia'
  `);
  console.table(alloc.rows);

  console.log("\n--- SALES (Last Month) ---");
  const sales = await client.query(`
    SELECT c.name as company, m.name as model, s.units_sold, s.revenue, s.main_reason
    FROM manufacturing_sales_results s
    JOIN manufacturing_vehicle_models m ON s.vehicle_model_id = m.id
    JOIN companies c ON m.company_id = c.id
    WHERE c.id IN (${ids}) AND s.world_year = 7 AND s.world_month = 4 AND s.region_market_id = 'drennia'
  `);
