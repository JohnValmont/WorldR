...
const orig = fs.readFileSync('temp_arc.ts', 'utf8');
...
let newCode = `
  ...
  private static async produceForCompany(trx: any, company: any, clock: any): Promise<ParticipantState> {
    ...
    // Warranty is calculated after sales, so we just initialize it to 0 here. Wait! Warranty reserve requires units sold!
    // Since sales haven't happened yet, we cannot compute warranty reserve during production!
    // We will compute it in settleForCompany.
...
fs.writeFileSync('temp_helpers1.ts', newCode);
