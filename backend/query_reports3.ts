...
async function main() {
  ...
  try {
    ...
    const reports = await client.query(`SELECT world_year, world_month, units_produced, units_sold FROM manufacturing_arc_reports WHERE company_id = $1 ORDER BY world_year DESC, world_month DESC LIMIT 3`, [compId]);
