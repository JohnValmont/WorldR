...
BEGIN
  ...
  IF haulp_id IS NOT NULL THEN UPDATE company_finances SET available_cash = 20000000 WHERE company_id = haulp_id; END IF;
  IF verid_id IS NOT NULL THEN UPDATE company_finances SET available_cash = 20000000 WHERE company_id = verid_id; END IF;
  IF apex_id  IS NOT NULL THEN UPDATE company_finances SET available_cash = 10000000 WHERE company_id = apex_id;  END IF;
  IF val_id   IS NOT NULL THEN UPDATE company_finances SET available_cash = 10000000 WHERE company_id = val_id;   END IF;
