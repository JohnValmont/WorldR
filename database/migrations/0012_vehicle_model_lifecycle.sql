...
-- Adds: development_type, facelift relationship, launch/discontinue arc tracking
-- Creates: manufacturing_model_snapshots table for per-model per-arc performance
...
ALTER TABLE manufacturing_vehicle_models
  ...
  ADD COLUMN IF NOT EXISTS launched_orbit INT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS launched_arc INT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS discontinued_orbit INT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS discontinued_arc INT DEFAULT NULL;
...
-- 2. Model performance snapshots (one record per company + model + arc)
CREATE TABLE IF NOT EXISTS manufacturing_model_snapshots (
    ...
    CONSTRAINT uq_model_snapshot UNIQUE (company_id, model_id, world_orbit, world_arc)
);
