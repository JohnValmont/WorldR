-- WORLDr Migration 0052: Hard Reset NPC Market Caps and Share Price History
-- The previous migration only deleted the current month's bar. If the world clock
-- was rolled back during testing, "future" bars (e.g. Year 10) remained in the DB,
-- causing the leaderboard CTE to pick up massively inflated future prices (e.g. $400B market caps).
-- This migration wipes ALL price history for NPCs and rebuilds a single current bar based on book value.
...
BEGIN
  ...
  -- 3. Calculate true book value per share
  ...
  -- 4. Set company_value to exact calculated market cap
  ...
  -- 5. WIPE ALL share price history for NPCs so there are no "future" rows sticking around
  DELETE FROM share_price_history WHERE company_id IN (haulp_id, verid_id, apex_id, val_id);

  -- 6. Insert a single clean bar for the CURRENT arc
  IF haulp_id IS NOT NULL THEN
