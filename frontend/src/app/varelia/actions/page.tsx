...
const POSITION_DEFINITIONS: Omit<Position, 'filledBy'>[] = [
  ...
  },
  {
    ...
    actions: [
      { id: 'buildCampaignInfrastructure', name: 'Build Campaign Infrastructure', description: 'Invest heavily in your ground operation. Train volunteers, set up regional field offices, optimize voter data collection, and build the organizational muscle needed to get out the vote on election day.', category: 'Campaign' },
...
function DutyRow({ action, positionTitle, accentColor, isFilled, onTrigger, ctx }: { action: PartyAction; positionTitle: string; accentColor: string; isFilled?: boolean; onTrigger?: (id: string) => void; ctx?: PlayerCtx }) {
  ...
  const implementedIds = ['mo_recruit', 'smallDonationDrive', 'pl_promise', 'cm_rally', 'meo_statement', 'doorToDoorCampaign', 'giveInterview', 'openMembershipBooth', 'pim_redo_char', 'pim_rebrand_party', 'pl_dissolve', 'cm_survey', 'buildCampaignInfrastructure'];
...
function ActionExecutionModal({
  ...
  const implementedIds = ['mo_recruit', 'smallDonationDrive', 'pl_promise', 'cm_rally', 'meo_statement', 'doorToDoorCampaign', 'giveInterview', 'openMembershipBooth', 'cm_survey', 'buildCampaignInfrastructure'];
  ...
  if (actionId === 'buildCampaignInfrastructure') {
    baseTiers = [{ base: 75000, mult: 1.00 }];
  }
  ...
  const canExecute = canAfford && isPromiseValid;
    ...
    } else if (actionId === 'buildCampaignInfrastructure') {
        campaignStrengthGain = 30.0;
        membersJoined = 2;
        publicTrustGain = 1.0;
...
export default function ActionsPage() {
  ...
  const handleActionExecute = (result: any) => {
    ...
    try {
      ...
      } else if (result.actionId === 'buildCampaignInfrastructure') {
        summaryStr = `Campaign & Media Manager built campaign infrastructure, increasing campaign strength.`;
