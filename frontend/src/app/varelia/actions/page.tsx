...
function GovernmentView({ ctx, onNavigateElections }: { ctx: PlayerCtx, onNavigateElections: () => void }) {
  ...
  if (!pastElection || !govRecord) {
    return (
      <div className="flex flex-col items-center justify-center p-8 h-full" style={{ background: BG }}>
        ...
        <button onClick={onNavigateElections} className="px-5 py-2.5 text-xs font-bold uppercase tracking-widest transition-opacity hover:opacity-80"
  ...
  const renderSeatChart = () => {
    ...
    for(let i=0; i<pastElection.independentIndividuals; i++) {
              ...
              <div className="w-full md:w-72 p-5 rounded-sm flex flex-col items-center justify-center" style={{ background: PANEL, border: `1px solid ${BORDER}` }}>
                ...
                <div className="w-full mt-6 space-y-2">
                  ...
                  <div className="flex justify-between items-center text-[10px] uppercase font-mono">
                    ...
                    <span className="font-bold text-zinc-400">{formatNumberUS(pastElection.independentIndividuals)}</span>
                  </div>
                  <div className="flex justify-between items-center text-[10px] uppercase font-mono">
                ...
                <table className="w-full text-left text-[11px]">
                  ...
                  <tbody>
                    ...
                    <tr className="border-b last:border-0 bg-white/[0.01]" style={{ borderColor: BORDER }}>
                      ...
                      <td className="py-3 text-right font-bold text-zinc-400">{formatNumberUS(pastElection.independentIndividuals)}</td>
                      <td className="py-3 text-right text-zinc-500 font-mono">{pastElection.notaPercent?.toFixed(1) || '0.0'}%</td>
                      <td className="py-3 text-right text-zinc-500 font-mono">{((pastElection.independentIndividuals / pastElection.parliamentSeats) * 100).toFixed(1)}%</td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-sm border-l-2" style={{ background: PANEL2, borderLeftColor: ACCENT, borderTop: `1px solid ${BORDER}`, borderRight: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
                ...
                <p className="text-[10px] text-zinc-400 leading-relaxed">
                  Ordinary bills will require <strong className="text-zinc-200">{formatNumberUS(pastElection.majoritySeats)} Yes votes</strong> to pass in Drennia’s {formatNumberUS(pastElection.parliamentSeats)}-seat parliament.
                </p>
              </div>
            </div>
          </div>
...
export default function ActionsPage() {
  ...
  const [activeSubtab, setActiveSubtab] = useState<'Party HQ' | 'Party Staff' | 'Party Strategy' | 'Voter Blocs' | 'Budget' | 'Elections' | 'Government' | 'Past Elections' | 'Activity Log'>('Party HQ');
      ...
      <div className="h-screen flex flex-col overflow-hidden transition-opacity duration-500"
        ...
        <div className="shrink-0 flex items-center px-4 md:px-5 border-b overflow-x-auto"
          ...
          <div className="flex gap-2 h-full">
            {(['Party HQ', 'Party Staff', 'Party Strategy', 'Voter Blocs', 'Budget', 'Elections', 'Government', 'Past Elections', 'Activity Log'] as const).map((tab) => {
        ...
        <div className="flex-1 overflow-hidden">
          {activeSubtab === 'Party HQ' ? (
            ...
            <GovernmentView ctx={ctx} onNavigateElections={() => setActiveSubtab('Elections')} />
