      ...
      <div className="flex items-center gap-1 overflow-x-auto">
        {JURISDICTIONS.map((j) => {
          ...
          return (
            <button
              key={j.id}
              onClick={() => onChange(j.id as JurisdictionId)}
            ...
            >
              {icon}
              <span>{j.name}</span>
              <span className={`font-mono text-[10px] px-1.5 py-0.5 rounded ${isSelected ? 'bg-amber-400/20 text-amber-200' : 'bg-zinc-800 text-zinc-400'}`}>
                {seats} seats
              </span>
            </button>
          );
