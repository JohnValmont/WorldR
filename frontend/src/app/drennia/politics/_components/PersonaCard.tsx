...
export default function PersonaCard({
  ...
  return (
    <div className="p-3.5 border border-[#2A2630] bg-[#11131A] rounded-sm flex flex-col gap-2">
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0">
          <div className="text-[#F4EBD6] text-sm truncate">{persona?.nickname || label}</div>
          <div className="text-[10px] text-[#A79D8C] truncate">{label}</div>
        </div>
        <span className="ml-auto text-[#E4DBCA] font-mono text-sm shrink-0">
          {(size * 100).toFixed(0)}%
        </span>
      </div>

      <div className="text-[10px] font-mono uppercase tracking-[0.1em] text-[#8F857A]">
        {persona?.lean}
      </div>
      ...
      {leadingParty != null && (
        <div className="pt-2 border-t border-[#2A2630]">
          <div className="flex items-center justify-between text-[10px] mb-1">
            <span className="text-[#A79D8C] uppercase tracking-wider">Leading</span>
            <span className="text-[#F4EBD6] truncate ml-2">{leadingParty}</span>
          </div>
          <div className="h-1.5 w-full bg-[#17151B] rounded-full overflow-hidden">
            <div
