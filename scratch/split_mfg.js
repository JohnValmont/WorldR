...
updatedCode = updatedCode.replace(/(import .* from '.*';\\n)+/, match => match + addedImports);

// Wrap main with provider
const ctxValueObj = \`
  const ctxValue = {
    company, mfgData, playerCash, characterName, onRefresh, isAdmin,
    deskTab, setDeskTab, notification, setNotification, bootstrapData, setBootstrapData,
...
updatedCode = updatedCode.replace(/return \\(\\s*<div className="flex flex-col h-screen overflow-hidden text-zinc-300">/, match => ctxValueObj + '\\n  return (\\n    <ManufacturingContext.Provider value={ctxValue}>\\n      <div className="flex flex-col h-screen overflow-hidden text-zinc-300">');
