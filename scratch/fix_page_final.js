...
// 1. Wrap StartBusinessTab return
p = p.replace(
  /const total = chosenCapital \+ FILING_FEE;\s*const canAfford = playerCash >= total;\s*return \(\s*<div style=\{\{ maxWidth: '620px' \}\}>/m,
...
// 2. Add missing closing div for StartBusinessTab
p = p.replace(
  /          <\/div>\s*<\/div>\s*\)\}\s*<\/div>\s*\{\/\* Right Rail - Filing Summary \*\/\}/m,
  \`          </div>
        </div>
      )}
        </div>
...
// 3. Fix market subtab to exchange subtab
p = p.replace(
  /\{activeTab === 'companies' && <MyCompaniesTab \/>\}\s*\{activeTab === 'market' && <MarketTab playerCash=\{playerCash\} onRefresh=\{handleRefresh\} \/>\}/m,
  \`{activeTab === 'companies' && <MyCompaniesTab />}
      {activeTab === 'exchange' && <DrennportExchangeTab />}\`
);

p = p.replace(
  /\{activeTab === 'market' && <MarketTab playerCash=\{playerCash\} onRefresh=\{refreshAll\} \/>\}/m,
  \`{activeTab === 'exchange' && <DrennportExchangeTab />}\`
);

// 4. Update Desk Tab Types
p = p.replace(
...
// 5. Update DESK_TABS array
p = p.replace(
  /\{ id: 'contracts',  label: 'Contracts'  \},\s*\{ id: 'facilities', label: 'Facilities' \},/,
  \`{ id: 'contracts',  label: 'Contracts'  },
    { id: 'procurement',label: 'Procurement'},
    { id: 'facilities', label: 'Facilities' },\`
);

// 6. Update deskTab === 'contracts' render to include procurement
p = p.replace(
  /\{deskTab === 'contracts' && <ContractsTab \/>\}\s*\{deskTab === 'facilities' && <FacilitiesTab \/>\}/m,
...
// 8. Add Procurement Button to Fleet Tab
p = p.replace(
  /function FleetTab\(\{ fleet, setDeskTab \}: any\) \{\s*return \(\s*<div style=\{\{ display: 'flex', flexDirection: 'column', gap: '16px' \}\}>\s*<SectionHeader>Fleet Operations<\/SectionHeader>/m,
...
// 9. Replace Facilities Acquire Button with Procurement
p = p.replace(
...
// 10. Append Procurement and Exchange components
const newTabsCode = \`
// ─── PROCUREMENT TAB ──────────────────────────────────────────────────────────
