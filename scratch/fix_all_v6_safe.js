...
if (backNavStart > -1 && backNavEnd > -1) {
...
// 5. Rewrite inline fleet logic
const inlineFleetStartStr = "{deskTab === 'fleet' && (";
const inlineFleetEndStr = "{deskTab === 'contracts' && (";

const inlineFleetStart = c.indexOf(inlineFleetStartStr);
const inlineFleetEnd = c.indexOf(inlineFleetEndStr);

if (inlineFleetStart > -1 && inlineFleetEnd > -1) {
  const newFleetTabStr = `{deskTab === 'fleet' && (
  ...
  c = c.substring(0, inlineFleetStart) + newFleetTabStr + c.substring(inlineFleetEnd);
}

// 6. Fix AssetsTab to be a summary, and pass onOpenMarket
...
const inlineAssetsStartStr = "{deskTab === 'assets' && (";
const inlineAssetsEndStr = "{deskTab === 'records' && (";
const inlineAssetsStart = c.indexOf(inlineAssetsStartStr);
const inlineAssetsEnd = c.indexOf(inlineAssetsEndStr);

if (inlineAssetsStart > -1 && inlineAssetsEnd > -1) {
  const newAssetsTabStr = `{deskTab === 'facilities' && (
      ...
      `;
  c = c.substring(0, inlineAssetsStart) + newAssetsTabStr + c.substring(inlineAssetsEnd);
}

// 7. Rewrite the actual AssetsTab component
const functionAssetsTabStart = c.indexOf('function AssetsTab');
if (functionAssetsTabStart > -1) {
  const functionAssetsTabEnd = c.indexOf('function RegistryTab'); // Since it's the last one before MarketTab maybe? Let's use finding end.
  let functionAssetsTabEndIdx = functionAssetsTabStart;
  let depth = 0, started = false;
  for(let i=functionAssetsTabStart; i<c.length; i++){
    if(c[i]==='{'){ depth++; started=true; }
    if(c[i]==='}'){ depth--; if(started && depth===0){ functionAssetsTabEndIdx = i+1; break; } }
  }

  const newAssetsFunc = `function AssetsTab({ company, fleet, setDeskTab, onOpenMarket }: any) {
  ...
  c = c.substring(0, functionAssetsTabStart) + newAssetsFunc + c.substring(functionAssetsTabEndIdx);
}

// 8. Create FacilitiesTab
const facilitiesTabStr = `\nfunction FacilitiesTab({ company, onRefresh, showNotif }: any) {
...
if (mStart > -1) {
  let mEndIdx = mStart;
  let depth = 0, started = false;
  for(let i=mStart; i<c.length; i++){
    if(c[i]==='{'){ depth++; started=true; }
    if(c[i]==='}'){ depth--; if(started && depth===0){ mEndIdx = i+1; break; } }
  }
  ...
  c = c.substring(0, mStart) + newMarketTab + c.substring(mEndIdx);
}
