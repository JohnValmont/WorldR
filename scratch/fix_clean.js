...
const startBusDecl = "function StartBusinessTab({ step, setStep";
...
if (startBusIdx !== -1) {
  ...
  if (retIdx !== -1) {
    ...
    let startBusBody = c.substring(startBusIdx, nextCompIdx);
    ...
    if (lastIndex !== -1) {
      ...
      c = c.substring(0, startBusIdx) + startBusBody + c.substring(nextCompIdx);
    }
  }
}
...
// Find exactly MarketTab
let startMarket = c.indexOf("function MarketTab({");
let nextAfterMarket = c.indexOf("function BusinessPage() {");
if (startMarket !== -1 && nextAfterMarket !== -1) {
  const newTabsCode = \`
// ─── PROCUREMENT TAB ──────────────────────────────────────────────────────────
...
\`;
  c = c.substring(0, startMarket) + newTabsCode + c.substring(nextAfterMarket);
}
