...
// ΓöÇΓöÇΓöÇ Theme ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const T = {
  gold: '#d4af37',
  muted: '#888888',
  faint: '#444444',
...
// ΓöÇΓöÇΓöÇ Currency formatter is defined inside the component from mfgData.currencySymbol

// ΓöÇΓöÇΓöÇ Phase 3: Engineering Priority Definitions (frontend mirror) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const ENG_PRIORITIES = [
  { id: 'reliability',   label: 'Reliability',               icon: '≡ƒ¢í', desc: 'More testing, lower risk, better reliability score.' },
  { id: 'performance',   label: 'Performance',               icon: 'ΓÜí', desc: 'Higher power output, better perf score. Heavier and costlier.' },
  { id: 'fuel_economy',  label: 'Fuel Economy',              icon: 'Γ¢╜', desc: 'Lightweight focus, better fuel efficiency. Reduces weight.' },
  { id: 'comfort',       label: 'Comfort',                   icon: '≡ƒ¢ï', desc: 'Interior refinement, better appeal score.' },
  { id: 'practicality',  label: 'Practicality',              icon: '≡ƒôª', desc: 'Space and utility focus, better cargo score.' },
  { id: 'mfg_simplicity',label: 'Manufacturing Simplicity',  icon: '≡ƒöº', desc: 'Design for manufacture. Reduces production cost.' },
];
...
// Phase 3 live engineering preview ΓÇö mirrors engineeringEngine.ts logic
...
// ΓöÇΓöÇΓöÇ Reusable Atoms ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Tab type ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export default function ManufacturingDeskTab({ company, mfgData, playerCash, characterName, onRefresh, isAdmin }: any) {
  ...
  // ΓöÇΓöÇΓöÇ Data-driven config from API ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // Currency formatter ΓÇö uses company's currency symbol, not a hardcoded $
  const fm = (val: number) => `${currencySymbol}${Math.round(val).toLocaleString()}`;

  // State resolver ΓÇö uses statesForCountry from the API, not a hardcoded lookup
  ...
  useEffect(() => { /* liveScore sale price sync ΓÇö moved below */ }, [dClass, dPlatform, dEngine, dDrivetrain, dInterior, dSafety, dQuality]);
  ...
  // ΓöÇΓöÇ Handlers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ∩┐╜∩┐╜∩┐╜∩┐╜∩┐╜ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // Phase 3: Live engineering assessment ΓÇö computed here where engineerCount is available
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full min-h-[calc(100vh-120px)]">
              ...
              <DataRow
                ...
                value={
                  <Button
                    ...
                    disabled={hasFactory}
                  >
                    {hasFactory ? 'Γ£ô Complete' : 'Go to Facilities'}
                  </Button>
                }
              />
              <DataRow
                ...
                value={
                  ...
                  >
                    {hasModel ? 'Γ£ô Complete' : 'Go to R&D'}
                  </Button>
                }
              />
              <DataRow
                ...
                value={
                  ...
                  >
                    {hasActivePlan ? 'Γ£ô Complete' : 'Go to Production'}
                  </Button>
                }
              />
            </div>
            ...
            {latestReport ? (
              <div className="flex flex-col">
                ...
                {(() => {
                  ...
                  const up = d >= 0;
                  return <DataRow label="Momentum vs Last Month" value={`${up ? 'Γû▓ +' : 'Γû╝ ΓêÆ'}${fm(Math.abs(d))}`} valueVariant={up ? 'green' : 'red'} border={false} />;
                })()}
              </div>
            ) : (
              <UIEmptyState
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <PanelBox>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[9px] font-mono uppercase tracking-[0.12em] text-terminal-green">ΓùÅ Available</span>
                </div>
            ...
            <div className="flex flex-col gap-4">
              {factories.map((factory: any) => (
                <PanelBox key={factory.id}>
                  ...
                  <div className="mt-4">
                    <GhostButton onClick={() => setDeskTab('production')}>Open Production ΓåÆ</GhostButton>
                  </div>

                  {/* ΓöÇΓöÇ FACILITY GROWTH ΓöÇΓöÇ */}
                  {factory.factory_type_id === 'small-workshop' && (() => {
                    ...
                    if (showExpandConfirm && expandingFactoryId === factory.id) {
                      return (
                        <div className="mt-5 border-t border-zinc-800 pt-4">
                          <div className="text-[10px] font-mono text-terminal-amber tracking-[0.15em] uppercase mb-3">Workshop Expansion ΓÇö {EXP_MAX_LINES} Production Lines</div>
      ...
      {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
          R&D / DESIGN TAB ΓÇö PORTFOLIO PAGE
      ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
      {deskTab === 'design' && (() => {
        ...
        const devBadge = (status: string, devStage?: string) => {
          ...
          const stageSuffix = status === 'in_development' && devStage && devStage !== 'ready_to_launch'
            ? ` ΓÇö ${stageLabels[devStage] ?? devStage}` : '';
          return (
        ...
        // Detail scores ΓÇö use stored values directly from DB
        ...
        return (
          <div className="flex flex-col md:flex-row gap-6 md:gap-8">
            {/* ΓöÇΓöÇ INTERNAL SUB-NAV (LEFT CORNER) ΓöÇΓöÇ */}
            ...
            {designTab === 'portfolio' && (
              <div>
                {/* ΓöÇΓöÇ VEHICLE DETAIL PANEL ΓöÇΓöÇ */}
                {selectedModel && (
                  <div
                    ...
                    >
                      ...
                      {selectedModel.development_status === 'in_development' && (
                        <div className="rounded-md border border-terminal-amber/30 bg-terminal-amber/5 p-4 mb-5">
                          ...
                          <p className="text-xs leading-relaxed text-zinc-500 m-0">
                            {selectedModel.dev_stage === 'engineering' && 'Engineering Phase ΓÇö core design and systems engineering work.'}
                            {selectedModel.dev_stage === 'prototype' && 'Prototype Phase ΓÇö building and evaluating physical prototypes.'}
                            {selectedModel.dev_stage === 'testing' && 'Testing Programme ΓÇö road testing and durability validation.'}
                      ...
                      {selectedModel.development_status === 'launched' && (
                        <div className="flex items-center gap-2 rounded-md border border-terminal-green/30 bg-terminal-green/5 px-4 py-2.5 mb-5 text-xs text-terminal-green">
                          ...
                          Launched ΓÇö available for production assignment.
                        </div>
                      )}
                      ...
                      {/* Phase 3B ΓÇö Warranty Risk & Production Impact */}
                      {(() => {
                          ...
                          <div style={{ marginBottom: '20px' }}>
                            ...
                            {warrantyRisk === 'High' && (
                              <div style={{ marginTop: '6px', padding: '8px 12px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '2px', fontSize: '11px', color: T.red, lineHeight: 1.5 }}>
                                ΓÜá High warranty risk. Reliability below 55 incurs a running reserve deducted each Month. Increase reliability to reduce ongoing costs.
                              </div>
                            )}
                          </div>
                        );
                      ...
                      {/* Phase 3B ΓÇö Prototype Validation Issues */}
                      {(() => {
                          ...
                          <div style={{ marginBottom: '20px' }}>
                            ...
                            <div style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '2px', padding: '12px 14px' }}>
                              <div style={{ fontSize: '11px', color: T.red, fontFamily: 'monospace', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                Γ£ù Validation {resultClass} ΓÇö {extraArcs > 0 ? `+${extraArcs} Month` : ''}{extraCost > 0 ? ` +${(extraCost * 100).toFixed(0)}% Extra Cost` : ''}
                              </div>
                              {issues.map((issue: string, i: number) => (
                                <div key={i} style={{ fontSize: '11px', color: T.faint, paddingLeft: '8px', borderLeft: '2px solid rgba(239,68,68,0.4)', marginBottom: '4px', lineHeight: 1.6 }}>ΓÇó {issue}</div>
                              ))}
                            </div>
                          </div>
                        );
                        ...
                        <div style={{ marginBottom: '20px' }}>
                          ...
                          <PanelBox style={{ border: `1px solid ${T.blue}33` }}>
                            ...
                            <div style={{ marginBottom: '8px', padding: '8px', background: 'rgba(255,255,255,0.02)', borderRadius: '2px' }}>
                              ...
                              <div style={{ fontSize: '11px', color: T.faint, paddingLeft: '8px', borderLeft: `2px solid ${T.border}`, marginBottom: '2px' }}>ΓÇó Primary Weakness: {engReport.primaryWeakness}</div>
                              <div style={{ fontSize: '11px', color: T.faint, paddingLeft: '8px', borderLeft: `2px solid ${T.border}`, marginBottom: '2px' }}>ΓÇó Production Suitability: {engReport.productionSuitability}</div>
                              <div style={{ fontSize: '11px', color: T.faint, paddingLeft: '8px', borderLeft: `2px solid ${T.border}`, marginBottom: '2px' }}>ΓÇó Recommended Market: {engReport.recommendedMarket}</div>
                            </div>
                          </PanelBox>
                        </div>
                      )}
                        ...
                        if (engReportLoading) {
                          return (
                            <div style={{ marginBottom: '20px' }}>
                              ...
                              <PanelBox><div style={{ fontSize: '11px', color: T.faint }}>Retrieving report from the engineering bureauΓÇª</div></PanelBox>
                            </div>
                          );
                        }
                              ...
                              {Array.isArray(rpt.recommendations) && rpt.recommendations.length > 0 && (
                                <div style={{ padding: '8px', background: 'rgba(255,255,255,0.02)', borderRadius: '2px' }}>
                                  ...
                                  {rpt.recommendations.map((rec: string, i: number) => (
                                    <div key={i} style={{ fontSize: '11px', color: T.faint, paddingLeft: '8px', borderLeft: `2px solid ${T.border}`, marginBottom: '2px', lineHeight: 1.6 }}>ΓÇó {rec}</div>
                                  ))}
                                </div>
                              )}
                            </PanelBox>
                      ...
                      <PanelBox>
                        ...
                        <div style={{ fontSize: '12px', color: T.ivory, lineHeight: 1.8 }}>
                          ...
                          {selectedModel.development_status !== 'launched'
                            ? <span style={{ color: '#f59e0b' }}>ΓÜá Must be launched before assigning to a production line.</span>
                            : <span style={{ color: T.mint }}>Γ£ô Ready for production assignment.</span>
                          }
                        </div>
                      </PanelBox>
                ...
                {/* ΓöÇΓöÇ DESIGN FORM MODAL (Phase 3 Wizard) ΓöÇΓöÇ */}
                {showDesignModal && (
                  <div style={{ position: 'fixed', inset: 0, zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(3px)' }}
                    ...
                    <div style={{ width: '1020px', maxWidth: '98vw', maxHeight: '94vh', overflowY: 'auto', background: '#0d0d0d', border: `1px solid ${T.gold}55`, padding: '32px', position: 'relative' }}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <div>
                          <div style={{ fontSize: '18px', fontWeight: 700, color: T.gold, letterSpacing: '0.05em' }}>
                            {faceliftSourceModelId ? 'Develop Facelift' : designWizardStep === 1 ? 'Step 1 ΓÇö Vehicle Architecture' : 'Step 2 ΓÇö Engineering Direction'}
                          </div>
                          <div style={{ fontSize: '11px', color: T.muted, marginTop: '3px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                            R&D Desk ΓÇö {faceliftSourceModelId ? 'Facelift' : 'New Model'}
                          </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                          ...
                          <button onClick={() => { setShowDesignModal(false); setDesignWizardStep(1); }} style={{ background: 'none', border: 'none', color: T.muted, fontSize: '22px', cursor: 'pointer', lineHeight: 1 }}>Γ£ò</button>
                        </div>
                      </div>

                      {/* ΓöÇΓöÇ STEP 1: Vehicle Architecture ΓöÇΓöÇ */}
                      {designWizardStep === 1 && (
                      ...
                      {/* ΓöÇΓöÇ STEP 2: Engineering Direction ΓöÇΓöÇ */}
                      {designWizardStep === 2 && (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '28px' }}>
                          ...
                          <div>
                            ...
                            <PanelBox style={{ marginBottom: '20px', border: `1px solid rgba(212,175,55,0.25)` }}>
                              ...
                              {ENG_PRIORITIES.map(p => {
                                ...
                                return (
                                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '7px 0', borderBottom: `1px dotted ${T.border}` }}>
                                    ...
                                    <button onClick={() => adjustPri(-5)} style={{ background: 'none', border: `1px solid ${T.border}`, color: T.muted, width: '22px', height: '22px', cursor: 'pointer', fontSize: '14px', lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>ΓêÆ</button>
                              ...
                              {Math.abs(prioritySum - 100) > 2 && (
                                <div style={{ marginTop: '10px', fontSize: '11px', color: T.red }}>ΓÜá Priorities must sum to 100. Currently: {prioritySum}.</div>
                              )}
                            </PanelBox>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            ...
                            {liveScore.balanceFlags.length > 0 && (
                              <PanelBox style={{ border: `1px solid rgba(245,158,11,0.3)` }}>
                                <div style={{ fontSize: '11px', color: '#f59e0b', fontFamily: 'monospace', textTransform: 'uppercase', marginBottom: '8px' }}>ΓÜá Engineering Flags</div>
                      ...
                      <div style={{ marginTop: '28px', paddingTop: '20px', borderTop: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', gap: '16px' }}>
                        {!faceliftSourceModelId && designWizardStep === 1 && (
                          <GoldButton onClick={() => setDesignWizardStep(2)} disabled={!modelName.trim() || modelName.trim().length < 2} style={{ padding: '11px 28px', fontSize: '12px' }}>
                            Continue to Engineering Direction ΓåÆ
                          </GoldButton>
                        )}
                        ...
                        {!faceliftSourceModelId && designWizardStep === 2 && (
                          <GhostButton onClick={() => setDesignWizardStep(1)}>ΓåÉ Back</GhostButton>
                        )}
                    ...
                    <div style={{ width: '800px', maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', background: '#0d0d0d', border: `1px solid ${T.gold}55`, padding: '32px', position: 'relative' }}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '11px', color: T.muted, marginTop: '3px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.08em' }}>R&D Desk ΓÇö Evaluation</div>
                        </div>
                        <button onClick={() => setShowCompareModal(false)} style={{ background: 'none', border: 'none', color: T.muted, fontSize: '22px', cursor: 'pointer', lineHeight: 1 }}>Γ£ò</button>
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '24px' }}>
                        <div>
                          <select value={compareModel1} onChange={e => setCompareModel1(e.target.value)} style={{ width: '100%', padding: '9px 10px', background: '#0e0e0e', border: `1px solid ${T.border}`, color: T.ivory, fontSize: '13px', marginBottom: '16px' }}>
                            <option value="">ΓÇö Select Model A ΓÇö</option>
                        ...
                        <div>
                          <select value={compareModel2} onChange={e => setCompareModel2(e.target.value)} style={{ width: '100%', padding: '9px 10px', background: '#0e0e0e', border: `1px solid ${T.border}`, color: T.ivory, fontSize: '13px', marginBottom: '16px' }}>
                            <option value="">ΓÇö Select Model B ΓÇö</option>
                            {models.map((m: any) => <option key={m.id} value={m.id}>{m.name}</option>)}
                          </select>
                          {(() => {
                ...
                {/* ΓöÇΓöÇ PORTFOLIO PAGE HEADER ΓöÇΓöÇ */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                ...
                {/* ΓöÇΓöÇ VEHICLE MODEL CARDS ΓöÇΓöÇ */}
                {models.length === 0 ? (
                  <EmptyState
                    icon="≡ƒö¼"
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ fontSize: '10px', color: T.faint }}>View Details ΓåÆ</div>
                          </div>
                        </div>
                      );
                    })}
                    ...
                    return (
                      <div key={id} style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${T.border}`, padding: '16px', borderRadius: '2px', opacity: isLocked ? 0.6 : 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                          ...
                          {isCompleted && <div style={{ color: T.mint, fontSize: '12px', fontWeight: 600 }}>Γ£ô Completed</div>}
                ...
                <div>
                  ...
                  {!hasAny ? (
                    <div style={{ textAlign: 'center', padding: '48px 24px', border: `1px dashed ${T.border}`, borderRadius: '2px' }}>
                      <div style={{ fontSize: '32px', marginBottom: '12px' }}>≡ƒôÜ</div>
                            ...
                            <div key={p.id} style={{ marginBottom: '16px', padding: '14px', background: 'rgba(255,255,255,0.02)', border: `1px solid ${T.border}`, borderRadius: '2px' }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                ...
                                <div style={{ textAlign: 'right' }}>
                                  <div style={{ fontSize: '11px', fontFamily: 'monospace', color: T.gold, fontWeight: 700 }}>Lv.{level} ΓÇö {LEVEL_LABELS[Math.min(level, LEVEL_LABELS.length - 1)]}</div>
                              ...
                              {level >= 1 && (
                                <div style={{ fontSize: '10px', color: T.faint, marginTop: '6px', borderTop: `1px dashed ${T.border}`, paddingTop: '6px' }}>
                                  {p.id === 'reliability' && `Γåô ${level * 3}% engineering risk on reliability-focused projects`}
                                  {p.id === 'performance' && `Γåæ ${level * 2} performance score bonus on perf-focused projects`}
                                  {p.id === 'fuel_economy' && `Γåæ ${level * 2} fuel efficiency bonus on eco-focused projects`}
                                  {p.id === 'comfort' && `Γåæ ${level * 2} appeal bonus on comfort-focused projects`}
                                  {p.id === 'practicality' && `Γåæ ${level * 2} cargo score bonus on utility projects`}
                                  {p.id === 'mfg_simplicity' && `Γåô ${level * 2}% manufacturing cost on mfg-focus projects`}
                                </div>
                              )}
                            </div>
                          );
                      ...
                      <div>
                        ...
                        {engReputation ? (
                          <PanelBox style={{ border: `1px solid ${T.gold}33`, marginBottom: '20px' }}>
                            ...
                            {[
                              { id: 'reliability_rep', label: 'Reliability Rep.', icon: '≡ƒ¢í' },
                              { id: 'performance_rep', label: 'Performance Rep.', icon: 'ΓÜí' },
                              { id: 'fuel_efficiency_rep', label: 'Fuel Economy Rep.', icon: 'Γ¢╜' },
                              { id: 'comfort_rep', label: 'Comfort Rep.', icon: '≡ƒ¢ï' },
                              { id: 'practicality_rep', label: 'Practicality Rep.', icon: '≡ƒôª' },
                              { id: 'mfg_efficiency_rep', label: 'Mfg. Simplicity Rep.', icon: '≡ƒöº' },
                            ].map(field => {
                        ...
                        <div>
                          <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr] gap-3 mb-3">
                            <div>
                              ...
                              >
                                <option value="">ΓÇö Halt Production ΓÇö</option>
      ...
      {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
          STAFFING TAB
      ΓòÉΓòÉΓòÉ∩┐╜∩┐╜∩┐╜ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
      {deskTab === 'staff' && (
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-3">
            {staffRoles.map((roleDef: any) => {
              ...
              if (roleDef.id === 'factory-worker') {
                ...
                effectText = activeLinesCount === 0 ? 'No benefit ΓÇö no active lines' : `+${bonus}% production efficiency`;
      ...
      {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ∩┐╜∩┐╜ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
          FINANCE TAB
      ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
      {deskTab === 'finance' && (
        <div className="flex flex-col gap-5">
                        ...
                        <div className="flex-1">
                          ...
                          {isArcReport && (
                            <div className="text-[10px] text-terminal-blue mt-2 font-mono uppercase tracking-[0.05em]">
                              Click to view full Month report Γ₧ö
                            </div>
                          )}
                        </div>
                      </div>
                            ...
                            <table style={{ width: '100%', fontSize: '11px', textAlign: 'left', borderCollapse: 'collapse' }}>
                              <thead>
                                <tr style={{ color: T.muted, borderBottom: `1px solid ${T.border}` }}>
                                  ...
                                  <th style={{ padding: '8px 4px' }}>Awareness Before ΓåÆ After</th>
                                  <th style={{ padding: '8px 4px' }}>Trust Before ΓåÆ After</th>
                                ...
                                {arcBrandResults.map((br: any, idx: number) => {
                                  ...
                                  return (
                                    <tr key={idx} style={{ borderBottom: `1px solid #1a1a1a` }}>
                                      ...
                                      <td style={{ padding: '8px 4px', color: T.ivory }}>{br.awareness_before} ΓåÆ {br.awareness_after}</td>
                                      <td style={{ padding: '8px 4px', color: T.ivory }}>{br.trust_before} ΓåÆ {br.trust_after}</td>
