...
`const SUB_TABS: { id: SubTab; label: string; requiresCompany?: boolean }[] = [
  ...
  if (bPageStart === -1 || bPageEnd === -1) throw new Error('Could not find Subtabs & Breadcrumbs section');
...
`;
  ...
  if (backNavStart === -1 || backNavEnd === -1) throw new Error('Could not find Back Anchor section');
...
`{/* ── Back / Breadcrumb Navigation (Anchors) ── */}
  ...
  if (tStartAssets === -1) throw new Error('Could not find AssetsTab');
  const assetsTabEnd = findFunctionEnd(c, tStartAssets);
  if (assetsTabEnd === -1) throw new Error('Could not find end of AssetsTab');
  ...
  c = c.substring(0, tStartAssets) + newAssetsTab + c.substring(assetsTabEnd);

  // 6. Rewrite FleetTab
  const tStartFleet = c.indexOf('function FleetTab');
  if (tStartFleet === -1) throw new Error('Could not find FleetTab');
  const fleetTabEnd = findFunctionEnd(c, tStartFleet);
  if (fleetTabEnd === -1) throw new Error('Could not find end of FleetTab');
  ...
  c = c.substring(0, tStartFleet) + newFleetTab + c.substring(fleetTabEnd);

  // 7. Insert FacilitiesTab right before FleetTab definition
  const tStartFleetForInsert = c.indexOf('function FleetTab');
...
`;
  c = c.substring(0, tStartFleetForInsert) + newFacilitiesTab + c.substring(tStartFleetForInsert);

  // 8. Update CompanyDeskTab Props
  // Add onOpenMarket to CompanyDeskTab function signature
  c = c.replace(/function CompanyDeskTab\(\{([^\}]*)\}\) \{/, `function CompanyDeskTab({$1, onOpenMarket}: any) {`);
  c = c.replace(/<AssetsTab[^>]*\/>/, `<AssetsTab company={company} fleet={fleet} setDeskTab={setDeskTab} onOpenMarket={onOpenMarket} />`);
  c = c.replace(/<FleetTab[^>]*\/>/, `<FleetTab company={company} fleet={fleet} onRefresh={onRefresh} showNotif={showNotif} onOpenMarket={onOpenMarket} />`);
  
  // Conditionally render Facilities tab
  const assetsRenderIdx = c.indexOf("{deskTab === 'assets' && (");
  if (assetsRenderIdx === -1) throw new Error("Could not find deskTab === 'assets'");
  c = c.substring(0, assetsRenderIdx) + `{deskTab === 'facilities' && (\n        <FacilitiesTab company={company} onRefresh={onRefresh} showNotif={showNotif} />\n      )}\n\n      ` + c.substring(assetsRenderIdx);
  
  // In BusinessPage, pass onOpenMarket
  c = c.replace(/<CompanyDeskTab ([^\>]*) \/>/, `<CompanyDeskTab $1 onOpenMarket={() => { setActiveTab('market'); setSelectedCompanyId(company.id); }} />`);
  
  // 9. Rewrite MarketTab
  const mStart = c.indexOf('function MarketTab');
  if (mStart === -1) throw new Error('Could not find MarketTab');
  const mEnd = findFunctionEnd(c, mStart);
  if (mEnd === -1) throw new Error('Could not find MarketTab end');

  const newMarketTab = `function MarketTab({ playerCash, onRefresh }: { playerCash: number, onRefresh: () => void }) {
...
`;
  c = c.substring(0, mStart) + newMarketTab + c.substring(mEnd);

  fs.writeFileSync('frontend/src/app/drennia/business/page.tsx', c);
  console.log('All v6 patches applied successfully.');
} catch (e) {
